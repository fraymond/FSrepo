<template>
  ...
  <div class="mod-slideshow">
    ...
    <el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="width: 100%;">
      ...
      <!-- 内容 -->
      <el-table-column prop="cmcNote" header-align="center" align="center" label="内容"></el-table-column>
      <!-- 路径 -->
      <el-table-column prop="cmcUrl" header-align="center" align="center" label="路径"></el-table-column>
      <!-- 手机号 -->
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        // 发起请求
        API.other.bannerList(params).then(({data}) => {
          // 请求成功
          if (data && data.code === 0) {
            // 赋值操作
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${userIds.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          // 确定删除
          API.other.bannerDel(userIds).then(({data}) => {
            if (data && data.code === 0) {
              this.$message({
                message: '操作成功',
                type: 'success',
