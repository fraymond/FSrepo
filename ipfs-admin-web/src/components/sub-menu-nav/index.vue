<template>
  <el-submenu v-if="menuNav.list && menuNav.list.length >= 1" :data-idx="menuNav.menuId + ''" :index="menuNav.menuId + ''">
    ...
    <sub-menu-nav v-for="item in menuNav.list" :key="item.menuId" :menu-nav="item"></sub-menu-nav>
  </el-submenu>
  <el-menu-item v-else :index="menuNav.menuId + ''" :data-idx="menuNav.menuId + ''" @click="gotoRouteHandle(menuNav.parentId, menuNav.url)">
    <i :class="['site-sidebar__menu-icon', menuNav.icon ? menuNav.icon : 'fa fa-circle-o']"></i>
    <span>{{ menuNav.name }}</span>
  </el-menu-item>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 跳转到菜单导航对应路由
      gotoRouteHandle (pId, url) {
        var routeName = getRouteNameByUrl(url)
        if (/\S/.test(routeName)) {
          this.$router.push({ name: routeName, query: {parentId: pId} })
        }
      }
    }
  }
